...
const USAGE = `Usage:
  ...
  rivet protocols find <query> [--include-drafts] [--include-retired] [--project=<path>] [--json]
  rivet protocols show <slug> [--include-drafts] [--include-retired] [--expected-revision=<n> --expected-digest=<digest>] [--project=<path>] [--json]
  rivet protocols retire <slug> --expected-revision=<n> [--project=<path>] [--json]
  rivet protocols update <slug> --from=<path> --expected-revision=<n> [--publish] [--project=<path>] [--json]
  rivet work propose --project=<path> (--request=<file>|--request-text=<text>|--ticket=<id>|--host-context-json=<json>) (--decomposition=<file>|--decomposition-json=<json>) [--tracker=jira|linear] [--acceptance-criteria=<text>] [--json]
  ...
  rivet verify [--json]
  rivet evidence [--json]
  rivet feature propose --project=<path> (--request=<file>|--request-text=<text>|--ticket=<id>) [--tracker=jira|linear] [--acceptance-criteria=<text>] [--client=claude|codex] [--json]
  rivet feature start <run-id> --project=<path> --expected-version=<n> --proposal-digest=<sha256> [--json]
  rivet feature run --project=<path> (--request=<file>|--request-text=<text>|--ticket=<id>) [--tracker=jira|linear] [--acceptance-criteria=<text>] [--client=claude|codex]
...
async function defaultConfirmDelivery(preview) {
  ...
  try {
    const answer = await Promise.race([
      readline.question(preview?.proposal?.action === 'branch-publish' ? 'Publish this exact commit to the new branch shown above? [y/N] ' : preview?.proposal?.action === 'review-update' ? 'Apply this exact title and body update? [y/N] ' : preview?.proposal?.action === 'review-request' ? 'Create this exact review request? [y/N] ' : preview?.proposal?.action === 'tracker-transition' ? 'Apply this exact tracker status transition? [y/N] ' : preview?.proposal?.action === 'tracker-update' ? 'Post this exact delivery summary to the ticket shown above? [y/N] ' : preview?.proposal?.action === 'deploy' ? 'Deploy this exact commit to the environment shown above? [y/N] ' : 'Merge this exact commit using the method shown above? [y/N] '),
      new Promise(resolvePromise => { timer = setTimeout(() => resolvePromise(''), CONFIRMATION_TIMEOUT_MS); }),
    ]);
    return /^(?:y|yes)$/i.test(String(answer).trim());
  } catch { return false; }
